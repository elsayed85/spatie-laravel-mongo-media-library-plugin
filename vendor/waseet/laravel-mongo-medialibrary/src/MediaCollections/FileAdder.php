...
use Waseet\MediaLibrary\Conversions\ImageGenerators\Image as ImageGenerator;
use Waseet\MediaLibrary\HasMedia;
use Waseet\MediaLibrary\MediaCollections\Exceptions\DiskCannotBeAccessed;
use Waseet\MediaLibrary\MediaCollections\Exceptions\DiskDoesNotExist;
use Waseet\MediaLibrary\MediaCollections\Exceptions\FileDoesNotExist;
use Waseet\MediaLibrary\MediaCollections\Exceptions\FileIsTooBig;
use Waseet\MediaLibrary\MediaCollections\Exceptions\FileUnacceptableForCollection;
use Waseet\MediaLibrary\MediaCollections\Exceptions\UnknownType;
use Waseet\MediaLibrary\MediaCollections\File as PendingFile;
...
class FileAdder
{
    ...
    public function toMediaCollectionOnCloudDisk(string $collectionName = 'default'): MongoMedia
    {
        return $this->toMediaCollection($collectionName, config('filesystems.cloud'));
    }

    public function toMediaCollectionFromRemote(string $collectionName = 'default', string $diskName = ''): MongoMedia
    {
        ...
        if ($this->fileSize > config('mongo-media-library.max_file_size')) {
        ...
        $fileName = app(config('mongo-media-library.file_namer'))->originalFileName($sanitizedFileName);
    ...
    public function toMediaCollection(string $collectionName = 'default', string $diskName = ''): MongoMedia
    {
        $sanitizedFileName = ($this->fileNameSanitizer)($this->fileName);
        $fileName = app(config('mongo-media-library.file_namer'))->originalFileName($sanitizedFileName);
        $this->fileName = $this->appendExtension($fileName, pathinfo($sanitizedFileName, PATHINFO_EXTENSION));
    ...
    public function toMediaLibrary(string $collectionName = 'default', string $diskName = ''): MongoMedia
    {
    ...
    protected function determineDiskName(string $diskName, string $collectionName): string
    {
        ...
        return config('mongo-media-library.disk_name');
    }
    ...
    protected function attachMedia(MongoMedia $media)
    {
        if (! $this->subject->exists) {
            ...
            $class::created(function ($model) {
                $model->processUnattachedMedia(function (MongoMedia $media, self $fileAdder) use ($model) {
    ...
    protected function processMediaItem(HasMedia $model, MongoMedia $media, self $fileAdder)
    {
        ...
        if ($this->generateResponsiveImages && (new ImageGenerator())->canConvert($media)) {
            $generateResponsiveImagesJobClass = config('mongo-media-library.jobs.generate_responsive_images', GenerateResponsiveImagesJob::class);

            $job = new $generateResponsiveImagesJobClass($media);

            if ($customConnection = config('mongo-media-library.queue_connection_name')) {
                $job->onConnection($customConnection);
            }

            if ($customQueue = config('mongo-media-library.queue_name')) {
    ...
    protected function guardAgainstDisallowedFileAdditions(MongoMedia $media)
    {
    ...
    protected function checkGenerateResponsiveImages(MongoMedia $media)
    {
    ...
    protected function toMediaCollectionFromTemporaryUpload(string $collectionName, string $diskName, string $fileName = ''): MongoMedia
    {
