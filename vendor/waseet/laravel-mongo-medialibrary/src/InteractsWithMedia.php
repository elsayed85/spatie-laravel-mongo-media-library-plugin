...
use Waseet\MediaLibrary\Downloaders\DefaultDownloader;
use Waseet\MediaLibrary\MediaCollections\Events\CollectionHasBeenCleared;
use Waseet\MediaLibrary\MediaCollections\Exceptions\InvalidBase64Data;
use Waseet\MediaLibrary\MediaCollections\Exceptions\InvalidUrl;
use Waseet\MediaLibrary\MediaCollections\Exceptions\MediaCannotBeDeleted;
use Waseet\MediaLibrary\MediaCollections\Exceptions\MediaCannotBeUpdated;
use Waseet\MediaLibrary\MediaCollections\Exceptions\MimeTypeNotAllowed;
use Waseet\MediaLibrary\MediaCollections\FileAdder;
use Waseet\MediaLibrary\MediaCollections\FileAdderFactory;
...
use Waseet\MediaLibrary\Support\MediaLibraryPro;
use Waseet\MediaLibraryPro\PendingMediaLibraryRequestHandler;
...
trait InteractsWithMedia
{
    ...
    public static function bootInteractsWithMedia()
    {
        static::deleting(function (HasMedia $model) {
            ...
            $model->media()->cursor()->each(fn (MongoMedia $media) => $media->delete());
        });
    }

    public function media(): MorphMany
    {
        return $this->morphMany(config('mongo-media-library.media_model'), 'model');
    }
     ...
     * @param string[] $keys
     *
     * @return \Waseet\MediaLibrary\MediaCollections\FileAdder[]
     */
     ...
     *
     *
     * @throws \Waseet\MediaLibrary\MediaCollections\Exceptions\FileCannotBeAdded
     */
    public function addMediaFromUrl(string $url, array|string ...$allowedMimeTypes): FileAdder
    {
        ...
        $downloader = config('mongo-media-library.media_downloader', DefaultDownloader::class);
    ...
    public function getFirstMedia(string $collectionName = 'default', $filters = []): ?MongoMedia
    {
    ...
    protected function removeMediaItemsNotPresentInArray(array $newMediaArray, string $collectionName = 'default'): void
    {
        $this
            ...
            ->reject(fn (MongoMedia $currentMediaItem) => in_array(
                $currentMediaItem->getKey(),
                array_column($newMediaArray, $currentMediaItem->getKeyName()),
            ))
            ->each(fn (MongoMedia $media) => $media->delete());
    ...
    public function clearMediaCollectionExcept(
        string $collectionName = 'default',
        array|Collection|MongoMedia $excludedMedia = []
    ): HasMedia {
        if ($excludedMedia instanceof MongoMedia) {
        ...
        $this
            ->getMedia($collectionName)
            ->reject(fn (MongoMedia $media) => $excludedMedia->where($media->getKeyName(), $media->getKey())->count())
            ->each(fn (MongoMedia $media) => $media->delete());
     ...
     *
     *
     * @throws \Waseet\MediaLibrary\MediaCollections\Exceptions\MediaCannotBeDeleted
     */
    public function deleteMedia(int|string|MongoMedia $mediaId): void
    {
        if ($mediaId instanceof MongoMedia) {
    ...
    public function loadMedia(string $collectionName): Collection
    {
        ...
        return $collection
            ->filter(fn (MongoMedia $mediaItem) => $collectionName !== '*' ? $mediaItem->collection_name === $collectionName : true)
            ->sortBy('order_column')
            ->values();
    }

    public function prepareToAttachMedia(MongoMedia $media, FileAdder $fileAdder): void
    {
    ...
    public function registerMediaConversions(MongoMedia $media = null): void
    {
    }
    ...
    public function registerAllMediaConversions(MongoMedia $media = null): void
    {
        $this->registerMediaCollections();
